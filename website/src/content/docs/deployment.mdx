...
description: Practical deployment paths for Bub — direct, Docker, and Telegram.
sidebar:
  order: 6
---
...
Telegram configuration and runtime behavior are documented in [Channels → Telegram](/en/channels/telegram/).
...
:::caution
Do not set `BUB_HOME=/data` directly in `.env` with this compose file, or the host bind source will also become `/data`. If you want Bub runtime home to be `/data` in container, split variables first (for example `BUB_HOME_HOST` for host path) and then set `BUB_HOME=/data`.
:::
